$(document).ready(function () {
    var dataTable = $('#datatable-index').DataTable({
        paging: true,
        searching: true,
        ordering:  true,
...
$(document).on('click','.cs-remove', function (e) {
    e.preventDefault();
...
function statusupdate(url,status){
    $.ajax({
        ...
        success: function(response){
            if(response.status == "success"){
                var oldstatus         = (status == 'pending') ? "Responded" :  "Pending";
                var status_url        = "/auth/customer-inquiry/"+response.id+"/update/";
