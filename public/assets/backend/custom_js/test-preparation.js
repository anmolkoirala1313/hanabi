...
$(document).on('click','.cs-test-remove', function (e) {
    e.preventDefault();
    ...
    }).then(function(t)
    {
        t.value
            ...
            Swal.fire({
                title: "Cancelled",
                text: "Test preparation details was not removed.",
                icon: "error",
    ...
    if(status == '0'){
        ...
        }).then(function(t)
        {
            t.value
                ...
                Swal.fire({
                    title: "Cancelled",
                    text: "Test preparation status was not changed.",
                    icon: "error",
                    confirmButtonClass: "btn btn-primary mt-2",
                    buttonsStyling: !1
                });
    ...
    $.ajax({
        ...
        success: function(response){
            if(response.status == "success"){
                ...
                var status_url        = "/auth/test-preparation/"+response.id+"/update/";
                var replacementblock  = '#test-status-button-'+response.id;
                ...
                Swal.fire({
                    html: '<div class="mt-2">' +
                        ...
                        '<h4>Success !</h4>' +
                        '<p class="text-muted mx-4 mb-0">' +
                        'Test preparation Status has been updated .</p>' +
                        '</div>' +
                        '</div>',
                    timerProgressBar: !0,
                    timer: 2e3,
                ...
                Swal.fire({
                    html: '<div class="mt-2">' +
                        ...
                        '<h4>Oops...! </h4>' +
                        '<p class="text-muted mx-4 mb-0">' +
                        'Test preparation status could not be changed at the moment .</p>' +
                        '</div>' +
                        '</div>',
                    timerProgressBar: !0,
                    timer: 3000,
        ...
        error: function() {
            Swal.fire({
                html: '<div class="mt-2">' +
                    ...
                    '<h4>Warning...! </h4>' +
                    '<p class="text-muted mx-4 mb-0">' +
                    'Could not confirm the status of the Test preparation.</p>' +
                    '</div>' +
                    '</div>',
                timerProgressBar: !0,
                timer: 3000,
